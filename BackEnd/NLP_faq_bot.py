...
# Создаем роутер, который мы потом подключим в main.py
router = APIRouter()
    ...
    def respond(self, user_input: str) -> str:
        ...
        for i, faq_vec in enumerate(self.vectors):
            ...
            if score > best_score:
                best_score, best_idx = score, i
        if best_score >= self.threshold:
            return f"Ответ на вопрос: ''{self.questions[best_idx]}'' - " + self.answers[best_idx]
        return "❓ Не нашёл подходящего ответа."
...
# Привязываем эндпоинт к router
@router.post("/")
async def chat_endpoint(request: ChatRequest, fastapi_req: Request):
    # 1. Получаем ответ от бота
    answer = bot.respond(request.message)

    # 2. Парсим IP пользователя
    # client.host содержит IP. Если ты за прокси (например, Nginx),
    # лучше использовать headers.get("X-Forwarded-For")
    user_ip = fastapi_req.client.host

    try:
        # 3. Сохраняем всё в таблицу (включая user_ip)
        supabase.table("chat_history").insert({
            "message": request.message,
            "response": answer,
            "user_ip": user_ip
    ...
    return {"response": answer, "your_ip": user_ip}  # Возвращаем IP тоже для теста